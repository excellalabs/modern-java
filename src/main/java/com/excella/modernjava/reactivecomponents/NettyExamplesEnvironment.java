...
public class NettyExamplesEnvironment {

    private static final ConcurrentHashMap<Class, NettyExamplesEnvironment> envs = new ConcurrentHashMap<>();
    private static final ConcurrentHashMap<Class, NettyExamplesEnvironment> overrideEnvs = new ConcurrentHashMap<>();

    private volatile SocketAddress serverAddress;
    private final Logger logger;
    private Class<?> exampleClass;

    public NettyExamplesEnvironment(Class<?> exampleClass, Logger logger) {
        this.exampleClass = exampleClass;
        this.logger = logger;
    }

    private NettyExamplesEnvironment(Class<?> exampleClass) {
    ...
    public SocketAddress getServerAddress(Class<?> serverClass, String[] args) {
        ...
        NettyExamplesEnvironment serverEnv = getRegisteredEnvironment(serverClass);
    ...
    public static NettyExamplesEnvironment newEnvironment(Class<?> exampleClass) {
        NettyExamplesEnvironment override = overrideEnvs.get(exampleClass);
        if (null != override) {
            return override;
        }
        NettyExamplesEnvironment env = new NettyExamplesEnvironment(exampleClass);
        envs.put(exampleClass, env);
        return env;
    }

    public static void overrideEnvironment(Class<?> exampleClass, NettyExamplesEnvironment examplesEnvironment) {
        overrideEnvs.put(exampleClass, examplesEnvironment);
    }

    public static NettyExamplesEnvironment getRegisteredEnvironment(Class<?> exampleClass) {
